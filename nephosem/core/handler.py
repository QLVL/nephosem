...
class BaseHandler(Paralleler):
    ...
    * self.settings - settings dict
    ...
    def update_one_file(self, filename, data, **kwargs):
        ...
        # process file
        fname = os.path.splitext(os.path.relpath(filename, start=settings['corpus-path']))[0] # change 2023.04.07: assign fid based corpus path in settings (flexible softcoding)
